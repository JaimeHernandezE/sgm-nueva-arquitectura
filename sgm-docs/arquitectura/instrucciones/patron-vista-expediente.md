...
4. **El botón primario es personal:** la misma fila muestra `Completar formulario` al responsable habilitado y `Pendiente — acción de otro rol` (badge) a cualquier otro usuario. La UI refleja el permiso; la validación vive en el servidor.
   - **Panel de simulación (solo demo):** el prototipo expone dos selects por expediente — «Ver como rol» (códigos `adq.*` del catálogo P-24) y «Situar en sub-paso». Sin selección, la vista se mantiene en su estado actual. Con rol seleccionado, la acción del paso activo solo se habilita si el rol coincide con el responsable del sub-paso; con sub-paso seleccionado, los anteriores se asumen aprobados y los siguientes quedan pendientes. La selección viaja por query string (`rol`, `paso`) a las pantallas de sub-paso, que aplican el mismo criterio (controles deshabilitados + banner cuando el paso no está activo para ese rol). Control de demo — no sustituye autenticación ni RBAC real.

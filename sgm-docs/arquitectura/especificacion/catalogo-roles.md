...
|---|---|---|
| `adq.solped` | 1.x | `adq.solicitante` *(básico — destino = su unidad)*, `adq.solicitante_daf` *(DAF — elige unidad de destino)*, `adq.aprobador_unidad`, `adq.formulador_presupuesto`, `adq.firmante_cdp` |
...
| Solicitante DAF | `adq.solicitante_daf` | `adq.solped` | Finanzas › Abastecimiento (DAF) | [`1-solped.md`](../../modulos/adquisiciones/procesos-transversales/1-solped.md) §1.1 — puede cambiar `requesting_unit` y `destination_unit` a cualquier unidad del tenant |
...
### 3.1 Unidades al crear SOLPED (`requesting_unit` vs `destination_unit`)

Ambos roles pueden `createPurchaseRequest` / `submitPurchaseRequest`. Campos:

| Campo | Label (ES) | Significado |
|---|---|---|
| `requesting_unit` | Unidad solicitante | Unidad en nombre de la cual se tramita la SOLPED. |
| `destination_unit` | Unidad de destino | Unidad beneficiaria de la compra — para quién es la solicitud. |

| Rol | `requesting_unit` | `destination_unit` |
|---|---|---|
| `adq.solicitante` (**básico**) | Autoasignada = unidad del `RoleAssignment`; **no editable**. Si el payload trae otra → `REQUESTING_UNIT_OUT_OF_SCOPE`. | Autoasignada = misma que `requesting_unit`; **no editable**. Si el payload trae otra → `DESTINATION_UNIT_OUT_OF_SCOPE`. |
| `adq.solicitante_daf` (**DAF**) | Autoasignada = unidad DAF del `RoleAssignment` (default); **modificable** — cualquier `OrganizationalUnit` del tenant. | Autoasignada al default DAF; **modificable** — cualquier unidad del tenant. Obligatoria; permite indicar la unidad beneficiaria correcta. Asignar este rol solo a usuarios de DAF (Abastecimiento / Finanzas). |

> Un solicitante básico **no puede** cambiar solicitante ni destino a una unidad distinta de la propia. Tramitar con unidades distintas exige `adq.solicitante_daf` en DAF. La aprobación de jefatura (1.2) y el alcance de listado de unidad se resuelven por `destination_unit`.
...
| `adq.solicitante` | (+ lector) `createPurchaseRequest`, `submitPurchaseRequest`, `previewBudgetAvailability`, `requestBudgetFinancing` — `requesting_unit` y `destination_unit` = unidad del `RoleAssignment` (§3.1) |
| `adq.solicitante_daf` | Mismas operaciones que `adq.solicitante`; `requesting_unit` y `destination_unit` = **cualquier unidad del tenant** (modificables, §3.1). Lectura de expediente: tenant completo (como roles DAF). |
...
| `11-creacion-solped` | Solicitante (`adq.solicitante`) — unidades fijas a la propia; Solicitante DAF (`adq.solicitante_daf`) — selectores de solicitante y destino (§3.1) |
...
7. ~~¿Quién crea SOLPED por otras unidades?~~ → resuelto: `requesting_unit` + `destination_unit` + roles `adq.solicitante` (ambas fijas a la propia) vs `adq.solicitante_daf` (ambas modificables en el tenant, asignado en DAF) — §3.1.

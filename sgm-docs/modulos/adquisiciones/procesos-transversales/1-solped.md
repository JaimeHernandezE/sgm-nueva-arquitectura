...
**Entidad(es) y campos:**
- `PurchaseRequest` — `requesting_unit` (ref., **obligatorio** — autoasignado según `RoleAssignment`; scope por rol: `adq.solicitante` = solo su unidad; `adq.solicitante_daf` = cualquier unidad del tenant — [`catalogo-roles.md`](../../../arquitectura/especificacion/catalogo-roles.md) §3.1), `description` (texto, **obligatorio**), `justification` (texto, **obligatorio**), `requested_date` (fecha, **obligatorio**), `purchase_modality` (enum, **opcional**: `agile_purchase` \| `framework_agreement` \| `public_tender` \| `direct_procurement`), `founded_resolution_attachment` (`DocumentRef`, **obligatorio si** `purchase_modality = direct_procurement` — subida previa vía `storeDocument` del core), `currency` (enum, **obligatorio**, default `CLP`: `CLP` \| `UF` \| `UTM` \| `USD`), `proposed_budget_line_id` (ref. `BudgetLine`, **opcional**), `proposed_fiscal_year` (número, **opcional**), `status` (enum, **obligatorio**: `draft`)
...
| Guardar borrador | `createPurchaseRequest` | `REQUESTING_UNIT_OUT_OF_SCOPE` | `requesting_unit` | No puede indicar una unidad distinta a la de su asignación de rol. | blocking | integridad:alcance_rbac |
...
| Enviar a aprobación | `submitPurchaseRequest` | `REQUESTING_UNIT_OUT_OF_SCOPE` | `requesting_unit` | No puede indicar una unidad distinta a la de su asignación de rol. | blocking | integridad:alcance_rbac |
...
- Solicitante básico (`adq.solicitante`) envía `requesting_unit` distinta a la de su `RoleAssignment` → `REQUESTING_UNIT_OUT_OF_SCOPE` (`severity: blocking`). Solicitante DAF (`adq.solicitante_daf`) puede elegir cualquier unidad del tenant.

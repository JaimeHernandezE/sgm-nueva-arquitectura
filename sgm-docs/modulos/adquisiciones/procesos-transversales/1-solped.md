...
| Rol | Solicitante ([`adq.solicitante`](../../../arquitectura/especificacion/catalogo-roles.md)) — unidades fijas a la propia; o Solicitante DAF ([`adq.solicitante_daf`](../../../arquitectura/especificacion/catalogo-roles.md)) — puede cambiar **unidad solicitante** y **unidad de destino** ([`catalogo-roles.md`](../../../arquitectura/especificacion/catalogo-roles.md) §3.1) |
| Plataforma | SGM |
| Optativo | Falso |
...
| Guardar borrador | `createPurchaseRequest` | `REQUESTING_UNIT_OUT_OF_SCOPE` | `requesting_unit` | No puede indicar una unidad solicitante distinta a la de su asignación de rol. | blocking | integridad:alcance_rbac |
...
| Guardar borrador | `createPurchaseRequest` | `MISSING_REQUIRED_FIELD` | `title` | El campo Título es obligatorio. | blocking | integridad:campo_requerido |
...
| Enviar a aprobación | `submitPurchaseRequest` | `REQUESTING_UNIT_OUT_OF_SCOPE` | `requesting_unit` | No puede indicar una unidad solicitante distinta a la de su asignación de rol. | blocking | integridad:alcance_rbac |
...
| Enviar a aprobación | `submitPurchaseRequest` | `MISSING_REQUIRED_FIELD` | `title` | El campo Título es obligatorio. | blocking | integridad:campo_requerido |
...
- Solicitante básico (`adq.solicitante`) envía `requesting_unit` o `destination_unit` distinta a su `RoleAssignment` → `REQUESTING_UNIT_OUT_OF_SCOPE` / `DESTINATION_UNIT_OUT_OF_SCOPE` (`severity: blocking`). Solicitante DAF (`adq.solicitante_daf`) puede cambiar ambas a cualquier unidad del tenant (default = DAF).

...
**Entidad(es) y campos:**
- `GoodsReceipt` — `procurement_case_id` (ref., **obligatorio**), `purchase_order_id` (ref., **obligatorio**), `receipt_type` (enum, **obligatorio**), `received_by` (ref., **obligatorio**), `receiving_unit` (ref., **obligatorio**), `received_date` (fecha, **obligatorio**), `service_period_start` / `service_period_end` (fecha, **obligatorio si** servicio recurrente), `supporting_document_ref` (ref., **obligatorio si** `receipt_type = service`), `status` (enum, **obligatorio**), `observations` (texto, **obligatorio si** no conforme)

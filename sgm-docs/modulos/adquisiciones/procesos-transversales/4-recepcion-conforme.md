...
**Entidad(es) y campos:**
- `GoodsReceipt` (nueva) — `procurement_case_id`, `purchase_order_id` (ref. `PurchaseOrder`), `receipt_type` (enum: `physical_good` \| `service`), `received_by` (ref. `User`), `receiving_unit`, `receipt_date`, `service_period_start` / `service_period_end` (**obligatorios si** `service` recurrente), `supporting_document_ref` (ref. almacenamiento de objetos, **obligatorio si** `service`), `status` (enum: `draft` \| `confirmed` \| `rejected` \| `partially_rejected`)
- `GoodsReceiptLine` *(confirmada)* — `goods_receipt_id`, `purchase_order_line_ref`, `quantity_ordered`, `quantity_received`, `quantity_accepted`, `quantity_rejected`, `rejection_reason` (**obligatorio si** `quantity_rejected > 0`)

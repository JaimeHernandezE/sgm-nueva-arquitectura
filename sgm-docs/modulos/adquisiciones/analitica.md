...
| `requesting_unit` | `PurchaseRequest.requesting_unit` → `OrganizationalUnit` | Unidades del municipio | Depende de **X-49** |
...
| `amount_accrued` | Suma de devengados | ⚠ | **Bloqueada por X-46** — dos definiciones de devengado no reconciliadas; esta medida no se publica hasta resolverlo (§9.1) |
| `amount_paid` | Suma de pagos materializados | Fecha de pago | Propuesta — frontera Pago/Tesorería **X-47 cerrada** (pago = Tesorería) |

⚠ **Regla de integridad:** las medidas financieras de distintas fases (solicitado → preobligado → comprometido → devengado → pagado) forman un embudo; el catálogo debe declarar explícitamente por qué pueden no cuadrar entre sí (desviaciones de monto —pendiente transversal de tolerancias—, cancelaciones, regularizaciones **X-40**). Sin esa nota, todo tablero generará la pregunta "¿por qué no suman lo mismo?" y erosionará confianza.
...
| `on_time_rate` | % de pasos cerrados dentro del plazo configurado | Cierre del paso | ⚠ Depende de timers configurables (**X-33**): sin plazo definido no hay "a tiempo" |
| `rejection_rate` | % de SOLPEDs / decisiones de modalidad rechazadas en aprobación | Fecha del rechazo | Propuesta |

**Candidatas futuras, no v0:** concentración de proveedores e indicador de fraccionamiento (conecta con la regla V6 del gateway y **X-35**) — valiosas pero sensibles; requieren definición con DM/jurídica antes de exponerlas como cifra oficial.
...
3. **Visibilidad por rol:** ⚠ punto a resolver (§9.3) — ¿un usuario de una unidad solicitante ve agregados de todo el municipio o solo de su unidad? La decisión afecta el contrato (filtro forzado por rol) y debe ser consistente con el catálogo RBAC (**X-24**).
4. **Personas naturales:** la dimensión `supplier` se expone solo para proveedores persona jurídica en v0; el tratamiento de proveedores persona natural queda sujeto al catastro de datos personales (**X-27**, Ley 21.719).

...
- Cambio de modalidad **después** de confirmada → no permitido por esta operación; requiere reversión formal del expediente. Ver pendiente.

> ⚠ **Pendiente de definir:** procedimiento de reversión de modalidad post-confirmación (¿anulación de `CaseStep` instanciados con auditoría y nueva `ModalityDecision`?) — candidato a regla transversal del expediente. Ventana y criterios de la detección de fraccionamiento (V6) — validar con DM y jurídica antes de incorporar a bases. Catálogo estructurado de causales de Trato Directo (artículo y literal) — derivable de Ley 19.886 y su reglamento. **Carga inicial de `NormativeParameter`:** los valores vigentes de tramos de licitación (V7) y umbrales de garantías (V8) según el reglamento post Ley 21.634 deben confirmarse contra la norma actualizada — con el diseño de parámetros configurables, la especificación ya no congela estos números: define los códigos de parámetro y exige su carga inicial verificada contra norma vigente como parte de la puesta en marcha.
...
> ⚠ **Pendiente de definir (prioritario, con DM):** si esta aprobación existe, para qué modalidades, si exige firma electrónica, y si aplica segregación decisor/aprobador. Mientras no se ratifique, el sub-paso se considera **Optativo = Verdadero** de forma provisional.

---

## 2.3 — Vinculación del proceso en Mercado Público
...
| Optativo | Falso *(No aplica para Trato Directo — ver detalle)* |

**Detalle:** Cierre de la etapa transversal: el usuario crea el proceso en Mercado Público (vía deep link, actuando directamente en el portal conforme al principio de integración solo lectura) y **registra en SGM el código/ID del proceso MP**, estableciendo el vínculo que habilita las lecturas de estado de ahí en adelante. Al validar el código, el expediente queda en estado vinculado y el flujo **deriva al subproceso de la modalidad** (etapa 3 en adelante, específica por modalidad). El identificador registrado varía según modalidad:

| Modalidad | Código que se registra | Origen |
|---|---|---|
| Compra Ágil | ID de Cotización | Deep link "Gestionar en MP" → usuario crea la cotización → copia el ID |
| Convenio Marco | ID de OC de catálogo (Compra Directa) o ID de Intención de Compra (Gran Compra) | Deep link al carro / Gran Compra |
| Licitación Pública | ID de la licitación | Usuario crea la licitación en MP (modo monitor desde el inicio) |
| **Trato Directo** | **No aplica en esta etapa.** La fase inicial de TD es 100% interna (resolución fundada, cotizaciones previas, eventual Toma de Razón); el código MP se registra en su propio subproceso al momento de la publicación (regla de publicidad en 24 horas). | — |

Registrado y validado el código, la SOLPED/expediente queda bloqueada en el estado correspondiente (ej. "En proceso de cotización" para Compra Ágil) y el seguimiento pasa a las lecturas de estado documentadas en `arquitectura/integracion-mercado-publico.md`.
...
- API de MP no disponible al validar → `MP_PROVIDER_UNAVAILABLE`; el vínculo no se persiste sin validación. ⚠ Ver pendiente sobre registro provisional.
- Usuario creó el proceso en MP pero no registró el código en SGM → expediente queda detenido al final de la etapa 2; candidato a timer de escalamiento (propiedad de flujos, `musts-arquitectura.md` §10.4).

> ⚠ **Pendiente de definir:** comportamiento ante `MP_PROVIDER_UNAVAILABLE`: ¿bloqueo total o registro provisional con flag `mp_link_unverified` y validación asíncrona al restablecerse la API? — misma familia del pendiente de caída de API de precios (1.1); candidato a regla transversal de resiliencia ante servicios externos. Plazo del timer de escalamiento para vínculo no registrado.
...
**Etapa anterior:** [1. SOLPED](./1-solped.md) · **Siguiente etapa:** subproceso específico de la modalidad confirmada (ej. [Compra Ágil — 3. Cotización](../1.%20compra-agil/3-cotizacion.md))

...
**Convención de pendientes:** este módulo usa el prefijo **T-nn**. Las referencias a Presupuestos (**P-nn**) y Contabilidad (**C-nn**) conservan su prefijo original.
...
| D-4 | **Profundidad** | Módulo completo, con el núcleo no diferible marcado explícitamente dentro |
...
> **PENDIENTE T-1 (estructural):** Especificar el contrato de entrada de órdenes de ingreso con sus dos modos —masivo anticipado y sincrónico— e inventariar los giradores reales de un municipio tipo, con volumen y sistema de origen. Sin esto, D-3 traslada riesgo en lugar de reducirlo. **Bloquea F3.**
...
Es inusual: el levantamiento normalmente describe la práctica, y aquí propone un cambio con justificación. **Debe recogerse como requisito**, no como comentario. La granularidad del traspaso a Contabilidad es una decisión de diseño con consecuencias directas sobre la conciliación.
...
**6. El control de vigencia de garantías lo dispara el sistema, no el usuario.** Según 41.2.4, Tesorería *«obtiene del sistema un listado de Garantías próximas a vencer y notifica a los departamentos correspondientes»*. Es un proceso proactivo, y de los pocos del corpus donde el sistema inicia la acción.

**7. Las garantías tienen una cadena de firma de tres personas y generan documento contable.** El proceso 41.1.1 indica firma de **Tesorera, Jefe de Contabilidad y Director de Finanzas**, y que el registro *«genera un documento contable de ingreso»*. En Odoo, `tesoreria.garantia` **no tiene enlace a `account.gov.move`**: caso de *expediente sin efecto de dominio* según la categoría definida en §3.2.1 del plan de Contabilidad.

**8. Los cheques tienen ciclo de vida propio con reemisión.** Además del cheque pagado, el proceso 38.2.6 cubre **cheques nulos** —documentos que no se pagarán— y **cheques caducados**, con emisión de un **nuevo decreto de pago** si el proveedor desea cobrar. La caducidad no cierra la obligación: la reabre por otra vía.

### 3.2 Odoo as-is

[`modelos-odoo.md`](modelos-odoo.md) reconstruye `tesoreria_gov_cl` y su portal. Muchos modelos usan el prefijo `account.gov.*` aunque viven en el addon de Tesorería.
...
| Consolidación con ciclo de depósito | `payment.consolidation`: `draft → rectifiable → reception → bank_deposit → done`; al pasar a recepción cierra las cajas |
| Decreto de pago con generación de asiento | `payment.decree` con `move_egreso_pagado_id`, reverso al cancelar, y débitos/créditos vía `contra_egreso_pagado_id` |
| Pago en lote | `payment.decree.batch` con líneas `pending / processed / error` y mensajes de error |
| Archivo de transferencia bancaria | `partner.bank.transfer.file` con códigos de método bancario |
| Caja chica anclada a decreto | `account.gov.petty.cash` con rendición y aprobación por línea |
| Garantías con renovación y catálogos | `tesoreria.garantia` con estados `draft → approved → collected / returned / renewed` |
| Recepción de datos externos | `sem.data.reception` crea orden de ingreso y pago automáticamente, con configuración por tipo de documento |
| Reajuste e intereses | `account.gov.ipc` mensual más `account.gov.interest.rate` |
| Portal de proveedores | Consulta de decretos de pago por el partner |

#### Qué Odoo hace mal o no hace

| Capacidad esperada | Situación real |
|---|---|
| **Garantía con efecto contable** | `tesoreria.garantia` **sin enlace a `account.gov.move`**, pese a que el proceso 41 exige documento contable de ingreso. Expediente sin efecto de dominio |
| **Asiento de la consolidación** | `_create_accounting_entry` existe pero **`action_done` no la invoca**. Código legado inactivo: la apariencia de cobertura sin la cobertura |
| Conciliación pago ↔ factura | `payment.line` legado; el O2M está **comentado** en el modelo y el flujo operativo no lo usa |
| **Conciliación bancaria auxiliar diaria** | Ausente. La conciliación existe en Contabilidad, mensual; la auxiliar de Tesorería no está modelada |
| **Certificado de Saldos Bancarios** | Ausente, pese a ser el artefacto que condiciona el cierre mensual de Contabilidad |
| **Estado Diario** | Ausente como entidad, pese a ser el documento que se firma y remite a Contabilidad (42.2.6 y 42.2.11) |
| **Anulación con ventana de un día** | Ausente. No hay entidad de anulación ni control de la ventana temporal |
| **Desistimiento del contribuyente** | Ausente |
| **FCM, Formulario 10, RMTNP, Ley de Alcoholes** | Ausentes. No hay pago a terceros ni aportes reglados |
| **Especies valoradas** | Ausentes |
| **Cheques nulos y caducados** | El modelo `account.gov.check` (en Contabilidad) tiene los estados, pero no el ciclo de reemisión por caducidad |
| Búsqueda de pagos pendientes por RUT | No evidenciada como capacidad de caja (40.2.6) |
...
| **Anulación y desistimiento con ventana** | **Sí (42.2.12)** | **No** | **Sin cobertura; regla temporal dura** |
| Depósito bancario al día hábil siguiente | Sí (42.2.6) | Parcial (`bank_deposit`) | Formalizar plazo |
| **Traspaso desagregado a Contabilidad** | **Sí, como requisito explícito (42.2.7)** | Resumen por cuenta | **Requisito nuevo; define granularidad del contrato** |
| Decreto de pago, transferencia y cheque | Sí (38) | Sí | Cubierto |
| **Cheques nulos y caducados con reemisión** | **Sí (38.2.6)** | Parcial (estados sin ciclo) | **Especificar reemisión** |
| **Conciliación bancaria auxiliar diaria** | **Sí (38.2.5)** | **No** | **Sin cobertura; frontera con Contabilidad resuelta en §3.1** |
| **Certificado de Saldos Bancarios** | Sí (proceso 33 de Contabilidad) | **No** | **Sin cobertura; bloquea el cierre mensual de Contabilidad** |
| **FCM y pagos a terceros** | **Sí (39)** | **No** | **Sin cobertura; plazos legales** |
| Garantías: ingreso, custodia, vigencia | Sí (41) | Sí | Cubierto |
| **Garantía con documento contable** | **Sí (41.1.1)** | **No** | **Expediente sin efecto de dominio** |
| Caja chica | No levantado | Sí | Odoo aporta el requisito |
| Recepción externa tipo SEM | No levantado | Sí | Odoo aporta el requisito; formalizar contrato |
| **Especies valoradas** | Mencionado (40) | **No** | **Sin cobertura** |
| **Órdenes de ingreso desde giradores** | Sí (40.2.3) | Parcial | **Contrato de entrada por especificar (T-1)** |

**Lectura.** Odoo cubre bien el núcleo operativo de caja y pago —es probablemente el módulo mejor implementado del as-is— y el levantamiento cubre bien el ciclo diario. La brecha se concentra en **tres frentes**: los artefactos que cruzan a Contabilidad (estado diario, certificado de saldos, conciliación auxiliar, efecto contable de garantías), los **pagos reglados a terceros** con plazo legal, y las **reglas temporales de anulación**.
...
|---|---|---|
| **LOCM art. 14 y DL 3.063 sobre rentas municipales** | El Fondo Común Municipal se constituye, entre otros, con el **62,5% de lo recaudado por permisos de circulación** | Porcentaje como `NormativeParameter` con vigencia temporal, clase «respaldo de órgano rector». Nunca constante en código |
...
| **Anulación de orden de ingreso** (42.2.12) | Solo el **mismo día de la transacción**, con formulario firmado que indica orden anulada y de reemplazo | Ventana temporal anclada al cierre de caja, no a 24 horas |
| **Custodia de garantías** (41) | Firma de Tesorera, Jefe de Contabilidad y Director de Finanzas; documento contable de ingreso; custodia física | Cadena de firma de tres roles; el documento físico existe y su custodia debe registrarse |
...
MT-3  Pagos                                (proceso 38; decreto, transferencia, cheque, reemisión)
...
2. **`PaymentDecree`** — la raíz del egreso. Ciclo propio, agrupa egresos devengados y produce el egreso pagado.
...
**Caja e ingreso:** `CashierSession`, `CashSessionBalance`, `RevenueCollection` (pago recibido), `CollectionItem`, `PaymentMethod`, `ValuedDocument` (especies valoradas), `CollectionAnnulment` (anulación y desistimiento, con ventana), `ExternalCollectionFeed` (recepción tipo SEM)

**Cuadratura:** `CashConsolidation`, `CashCount` (arqueo), `DailyStatement` (Estado Diario), `BankDeposit`

**Egreso:** `PaymentDecree`, `PaymentDecreeLine`, `PaymentBatch`, `Check` (con nulo, caducado y reemisión), `BankTransferFile`, `PettyCash`, `PettyCashAccountability`

**Terceros:** `ThirdPartyTransfer`, `FCMContribution`, `RegulatoryTransferForm` (Formulario 10), `TrafficFineRegistry` (RMTNP)

**Garantías:** `Guarantee`, `GuaranteeCustody`, `GuaranteeRenewal`, `GuaranteeExpiryAlert`, `GuaranteeCollection`
...
| **Giradores externos** | Externo → Tes/Cont | Órdenes de ingreso, en modo masivo anticipado y sincrónico (D-3, T-1) | **Crítica** |
...
> **PENDIENTE T-4:** Definir la granularidad del traspaso diario a Contabilidad. El levantamiento propone bajar de «resumen por cuenta contable» a al menos «tipo de pago», y sugiere evaluar el detalle por pago individual (42.2.7). Afecta volumetría, conciliación y capacidad de auditoría.
...
| Entregable | Detalle |
|---|---|
| **Contrato de entrada de órdenes de ingreso** | T-1: dos modos, inventario de giradores reales con volumen. Bloquea F3 |
| Frontera con Contabilidad | Confirmación de D-1; propiedad del certificado de saldos y de las dos conciliaciones |
...
| BPMN — Anulación y desistimiento | Ventana temporal, formularios, firmas, efecto sobre la caja del día |
...
| **Reclasificación con la lente de expediente sin efecto de dominio** | Aplicar la categoría de §3.2.1 del plan de Contabilidad a todos los procedimientos TUPA de Tesorería |
...
| MT-3 Pagos | 4 (recepción de decreto → transferencia → cheque → nulos, caducados y reemisión) |
| MT-4 Pagos reglados a terceros | 4 (FCM permisos → RMTNP Registro Civil → Ley de Alcoholes → transferencias a municipios) |
| MT-5 Garantías | 5 (ingreso y validación → custodia → control de vigencia → devolución o renovación → cobro) |
...
| Entregable | Detalle |
|---|---|
| Modelo de entidades consolidado | Con `Guarantee` enlazada a su efecto contable, corrigiendo el as-is |
| Máquinas de estado | Sesión de caja, consolidación, decreto, cheque con reemisión, garantía |
| Contratos de API inter-módulo | Los catorce de §6, versionados |
...
| Especificación de escalabilidad | Caja es el punto de mayor concurrencia con usuarios finales; la volumetría del traspaso diario depende de T-4 |
...
| **T-4** | Granularidad del traspaso diario a Contabilidad | F0 / F4 | Equipo + Contabilidad |
...
|---|---|---|
| **El contrato de entrada de órdenes de ingreso queda mal especificado** | **Muy alto — Caja no puede cobrar; es el único punto de falla que deja al municipio sin operar de cara al ciudadano** | T-1 resuelto en F0, con inventario real de giradores y volumen |
...
| Procedimientos TUPA sin efecto de dominio dados por cubiertos | Medio-alto — la garantía sin asiento y la consolidación sin `_create_accounting_entry` invocada son casos confirmados | Revisión con la lente de §3.2.1 del plan de Contabilidad, en F2 |
...
1. El contrato de entrada de órdenes de ingreso está especificado en sus dos modos y validado con al menos dos giradores reales.
2. Ninguna garantía, consolidación ni cobro figura como cubierto si solo existe como expediente sin efecto sobre el dominio.
...
5. La ventana de anulación está anclada al cierre de la sesión de caja y es verificable por el motor.
...
8. La cadena de firma de garantías con tres roles es impuesta por el motor.
...
2. **Caja chica, especies valoradas y recepción externa tipo SEM no están levantadas como procesos.** Caja chica y SEM provienen del as-is de Odoo; especies valoradas solo se menciona de pasada en la descripción del proceso 40. Los tres requieren levantamiento en F2 antes de especificarse.

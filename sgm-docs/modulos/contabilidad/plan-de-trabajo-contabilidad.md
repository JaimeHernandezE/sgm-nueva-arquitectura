...
| D-2 | **Alcance del módulo** | Incluye inventario y activo fijo (proceso 28), **factoring y cesión de facturas (proceso 32)**, **conciliación bancaria contable mensual** (proceso 37; distinta de la auxiliar diaria de Tesorería) y reportes externos a CGR y SINIM (procesos 33, 35, 36). **Nota B0/v0.2:** el alta del bien ocurre al **devengar la factura** (Normativa Contabilidad General). Default de **X-44** en plan general §8 = **(a)** incluir en Contabilidad/núcleo; (b) solo con compensación escrita (M2). |
...
Esto choca de frente con el principio de independencia modular con contratos versionados. **Orquestar las dos APIs en el camino feliz no resuelve el problema:** si una confirma y la otra falla (timeout, validación, caída), queda un estado ilegal a menos que exista un mecanismo único de atomicidad o compensación (quién orquesta, idempotencia/reintento, reversa auditada). No es un detalle de implementación: es probablemente **el requisito técnico más duro de toda la arquitectura SGM**, y hay que resolverlo en la especificación, no dejarlo al adjudicatario.

> **PENDIENTE C-1 (estructural):** Definir el mecanismo que garantiza la atomicidad del devengo dual entre Presupuestos y Contabilidad. Candidatos: (a) saga con compensación/reversa contable o presupuestaria si el segundo paso falla; (b) un único punto de commit / orquestador con el otro módulo como suscriptor obligatorio y reconciliación forzada; (c) otro mecanismo documentado por SUBDERE. Es el ancla operativa del ADR [`2026-07-atomicidad-efectos-borde.md`](../../arquitectura/decisiones/2026-07-atomicidad-efectos-borde.md) (tres manifestaciones). Requiere decisión arquitectónica, no solo de modelado. **Bloquea F4 de ambos módulos.** Registro: [`pendientes.md`](../../arquitectura/decisiones/pendientes.md) C-1.

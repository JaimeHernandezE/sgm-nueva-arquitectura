...
**Plataforma de Control y primer consumidor.** SGM publica sus APIs (core de plataforma y módulos) detrás de la Plataforma Institucional de Gestión de APIs (Plataforma de Control). El frontend base y un municipio à la carte son consumidores del mismo frente — sin canal interno. El core es nodo de clase plataforma (obligatorio); Adquisiciones es el primer módulo de negocio publicado en el catálogo del Nodo SUBDERE. Base técnica canónica: repositorio `nodo-subdere`, [`docs/plataforma-control.md`](https://github.com/jaimehernandeze/nodo-subdere/blob/main/docs/plataforma-control.md). La licitación de esa capa es aparte de SGM (Opción B de [`nodo-integracion-subdere.md`](arquitectura/decisiones/nodo-integracion-subdere.md) §11–§12); no se duplica aquí.

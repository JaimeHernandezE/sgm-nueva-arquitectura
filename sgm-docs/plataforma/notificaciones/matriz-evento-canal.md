# Matriz evento → canal → destinatario (borrador X-06)

Borrador derivable de fichas y [`modulos/adquisiciones/contracts.md`](../../modulos/adquisiciones/contracts.md) §4. **Estado:** borrador — cierra **[X-06]** tras validación con DM / roles ([`catalogo-roles.md`](../../arquitectura/especificacion/catalogo-roles.md)).
...
| rechazo SOLPED *(si se emite evento — verificar ficha)* | `info` | `adq.solicitante` | sí | obligatorio | — | Alinear nombre de evento al cerrar X-06 |
...
| `ProcurementModalityConfirmed` | `action_required` | `adq.aprobador_modalidad` **si** X-38 exige visto bueno; si no, `info` a gestor | sí | obligatorio si action | — | Condicionado a X-38 |
...
| `GoodsReceiptConfirmed` | `action_required` | `adq.operador_pago` o confirmante según X-46 | sí | obligatorio | — | Siguiente hito contable/pago |
...
| `AccrualRecorded` / `AccrualRegistered` | `info` | operador pago / Contabilidad (M2M) | sí | preferencia | — | Unificar al cerrar X-46 |
...
Los mismos eventos de dominio pueden entregarse a `EventSubscription` con scopes (X-05). No sustituyen bandeja. Destinatario = sistema suscrito, no rol de persona.

---

## 6. Huecos a cerrar con X-06
...
3. Unificación `AccrualRecorded` vs `AccrualRegistered` (X-46).
